    ...
    const checkAuth = async () => {
      try {
        ...
        if (token) {
          ...
          // No authentication token found; nothing to do
        }
      } catch (error) {
        localStorage.removeItem('accessToken');
        localStorage.removeItem('refreshToken');
    ...
    try {
      ...
      Object.entries(updates).forEach(([key, value]) => {
        if (key === 'profileImage' && value) {
          ...
          // arrays should be appended as JSON string
      ...
      // Don't manually set it - it will break multipart boundary
      const response = await api.patch(API_PATHS.AUTH.UPDATE_PROFILE, formData);
